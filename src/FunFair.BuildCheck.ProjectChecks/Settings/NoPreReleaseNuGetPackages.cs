...
public sealed class NoPreReleaseNuGetPackages : IProjectCheck
{
    ...
    public ValueTask CheckAsync(ProjectContext project, CancellationToken cancellationToken)
    {
        foreach (PackageReference package in project.ReferencedPackageElements(this._logger))
        {
            ...
            this.CheckReference(projectName: project.Name, privateAssets: privateAssets, package: package);
        }

        return ValueTask.CompletedTask;
    }

    private void CheckReference(string projectName, string? privateAssets, in PackageReference package)
    {
        if (
        ...
        this._logger.FoundNuGetPackageAtVersion(projectName: projectName, packageId: package.Id, version: version);

        if (!NuGetVersion.TryParse(value: version, out NuGetVersion? nuGetVersion))
        {
            this._logger.CouldNotParseVersion(projectName: projectName, packageId: package.Id, version: version);
        ...
        {
            this._logger.UsesPreReleaseVersion(projectName: projectName, packageId: package.Id, version: version);
        }
    }
}
