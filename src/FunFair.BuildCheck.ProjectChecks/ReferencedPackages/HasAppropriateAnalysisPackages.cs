...
public abstract class HasAppropriateAnalysisPackages : IProjectCheck
{
    ...
    {
        if (StringComparer.OrdinalIgnoreCase.Equals(x: this._detectPackageId, y: package.Id))
        {
            foundSourcePackage = true;
        }

        if (StringComparer.OrdinalIgnoreCase.Equals(x: this._mustIncludePackageId, y: package.Id))
        {
